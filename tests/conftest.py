...
2. Models are discovered, not hardcoded. The Schependomlaan example is committed
   ...
   The synthetic IFC4 fixture is committed too; it is a single known path rather
   than a discovery, because it is generated by this repo and there is exactly
   one of it.
"""
...
SYNTHETIC_MODEL = REPO_ROOT / "examples" / "data" / "synthetic" / "synthetic-ifc4.ifc"
...
def _model_params():
    ...
    if SYNTHETIC_MODEL.exists():
        params.append(pytest.param(SYNTHETIC_MODEL, id="synthetic",
...
def synthetic_model():
    """
    The generated IFC4 fixture. Committed, so absence is a broken checkout.

    test_synthetic_model.py reports that as a failure rather than a skip, for the
    same reason the open-access models do: a missing file would turn every test
    over it into a skip and leave CI green while covering nothing.
    """
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "synthetic: runs against the generated IFC4 fixture")
    config.addinivalue_line(
